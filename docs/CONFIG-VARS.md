...
| INGRESS_NGINX_CHART_VERSION | ingress-nginx helm chart version | string | "UNSPECIFIED" | false | If left as "UNSPECIFIED", version 3.20.1 will be used for K8s clusters whose version is <= 1.21.X and version 4.0.13 will be used for K8s clusters whose version is >= 1.22.X | baseline |
