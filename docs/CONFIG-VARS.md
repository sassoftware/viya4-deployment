...
| V4_CFG_SYSTEM_NODEPOOL_RESTRICTION | Restrict workloads from system/management nodepools | bool | true | false | Adds node anti-affinity to prevent workloads from scheduling on system/management nodes. Replaces Azure-specific labels with cloud-agnostic configuration (PMFR4PLTFND-760) | viya |
| V4_CFG_SYSTEM_NODEPOOL_LABEL | Label key for identifying system/management nodepool nodes | string | workload.sas.com/class | false | Configures the node label used for system nodepool anti-affinity. Use generic labels instead of cloud-specific ones (e.g., `workload.sas.com/class` vs `kubernetes.azure.com/mode`) | viya |
| V4_CFG_SYSTEM_NODEPOOL_VALUE | Label value for system/management nodepool nodes | string | system | false | Configures the label value that identifies system/management nodes to avoid scheduling workloads on | viya |
