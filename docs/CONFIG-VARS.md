...
| Name | Description | Type | Default | Required | Notes | Tasks |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| V4_CFG_SAS_API_KEY | SAS API Key| string | | false | [API credentials](https://developer.sas.com/guides/sas-viya-orders.html) can be obtained from the [SAS API Portal](https://apiportal.sas.com/get-started) (Refer Deprication note below) | viya |
| V4_CFG_SAS_API_SECRET | SAS API Secret | string | | false | [API credentials](https://developer.sas.com/guides/sas-viya-orders.html) can be obtained from the [SAS API Portal](https://apiportal.sas.com/get-started)(Refer Deprication note below) | viya |
...
| USE_APIM_CREDS | Switch to use APIM credentials instead of legacy SAS API credentials | bool | false | true | Set to `false` to use SAS API credentials. Supported until April 20, 2026, after which legacy SAS API credentials will no longer work. | viya |
