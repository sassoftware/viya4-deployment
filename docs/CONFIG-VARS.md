...
| V4_CFG_BASELINE_CREATE_BOOTSTRAP_GATEWAY | Whether baseline should create a cluster-side bootstrap Gateway | bool | true | false | Baseline-driven Gateway creation for clusters where Viya may be deployed separately/manual. Useful for OpenStack-style Gateway creation (`gw-system` / `k8s-gw`) before Viya HTTPRoutes are generated. | baseline |
| V4_CFG_BASELINE_GATEWAY_NAMESPACE | Namespace for baseline bootstrap Gateway | string | gw-system | false | Used when `V4_CFG_BASELINE_CREATE_BOOTSTRAP_GATEWAY=true`. | baseline |
...
| CERT_MANAGER_CHART_VERSION | cert-manager Helm chart version | string | 1.21.0 | false | | baseline |
...
By default, two block storage StorageClasses are created using the driver, one for RabbitMQ and one for Crunchy Postgres. These StorageClasses provision **zonal** Persistent Disks. They are not Regional PDs and are not zone-redundant. In multi-zone GKE deployments, if zone-redundant RWO block storage is required, disable managed StorageClass creation (`V4_CFG_MANAGE_STORAGE = false`) and create your own StorageClasses with `replication-type: regional-pd`. Refer to the [GCP Regional Persistent Disk documentation](https://cloud.google.com/compute/docs/disks/regional-persistent-disk) for details. The defaults for these StorageClasses are listed below.
