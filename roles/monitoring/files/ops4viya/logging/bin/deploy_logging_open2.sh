...
response=$(curl  -s -o /dev/null -w "%{http_code}"  -XPUT "https://localhost:$TEMP_PORT/_ingest/pipeline/viyaburstns" -H 'Content-Type: application/json' -d @logging/es/es_create_ns_burst_pipeline2.json  --user $ES_ADMIN_USER:$ES_ADMIN_PASSWD --insecure)
...
response=$(curl  -s -o /dev/null -w "%{http_code}" -XPUT "https://localhost:$TEMP_PORT/_template/viya-logs-template "    -H 'Content-Type: application/json' -d @logging/es/odfe/es_set_index_template_settings_logs2.json --user $ES_ADMIN_USER:$ES_ADMIN_PASSWD --insecure )
# TO DO/CHECK: this should return a message like this: {"acknowledged":true}
if [[ $response != 2* ]]; then
...
if [ "$FLUENT_BIT_ENABLED" == "true" ]; then
   ...
   logging/bin/deploy_logging_fluentbit_open2.sh
else
